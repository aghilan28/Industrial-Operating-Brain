...
def install_rest_exception_handlers(app) -> None:
    ...
    These handlers cover the REST-side routes (assets, alerts, dashboard, auth, etc.)
    while the existing ai_service handlers cover the /ai/* routes.
    """
    app.add_exception_handler(AppException, app_exception_handler)
    app.add_exception_handler(RequestValidationError, validation_exception_handler)
    # Only add StarletteHTTPException handler if the AI-service hasn't already
    # The AI service handler already covers StarletteHTTPException globally,
    # so we skip adding a duplicate. Our AppException handler is the key addition.
