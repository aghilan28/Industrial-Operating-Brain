...
class Settings(BaseSettings):
    ...
    # Base URL for the AI microservice platform (prediction, GraphRAG, LLM).
    AI_PLATFORM_URL: str = Field(
        default="http://localhost:8001",
        description="Root URL of the internal AI platform microservice.",
    )
    # Timeout (seconds) for AI client requests including connect, read, and
    # write.  A dedicated connect timeout of 5 s is also applied separately.
    AI_CLIENT_TIMEOUT: float = Field(
        default=10.0,
        description="Total request timeout for AI client calls (seconds).",
    )
    # ── End Phase 6 ────────────────────────────────────────────────────
